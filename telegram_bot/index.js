    ...
    try {
      if (bankSms.type === "EXPENSE") {
        ...
        let reply = `📲 *Bank SMS Auto-Parsed & Logged!*\n\n` +
          ...
          `🏦 *Account:* ${bankSms.account}\n` +
          `📁 *Category:* ${bankSms.category}\n` +
          (bankSms.balanceAfter !== null ? `💳 *Live Balance:* ₹${bankSms.balanceAfter.toLocaleString("en-IN", { minimumFractionDigits: 2 })}\n\n` : "\n") +
...
bot.launch().then(() => {
  console.log("🚀 OrbitLife AI Personal CFO Bot is RUNNING with Full Intelligence Suite!");
}).catch((err) => {
