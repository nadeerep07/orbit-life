...
// Main Keyboard Menu with 1-Tap Quick Access
const mainMenu = Markup.keyboard([
  ["🏦 Live Balances", "💳 Supermoney Card"],
  ["📑 EMIs & Dues", "🤝 Debts (Borrow/Lend)"],
  ["📊 Analytics & Stats", "🥗 Nutrition & Food"],
  ["❓ Help & Cheatsheet", "⚙️ Account Link"],
]).resize();
...
bot.start(async (ctx) => {
  ...
  let msg = `👋 *Welcome to OrbitLife Full Automation Bot!*\n\n` +
    `Every part of your personal finances can now be automated from Telegram (₹ INR):\n\n`;
  ...
  if (userId) {
    msg += `✅ *Connected to App:* \`${userId.substring(0, 8)}...\`\n\n` +
      ...
      `• *Live Balance:* Tap *🏦 Live Balances* or send _/balance_\n` +
      `• *Card Limits:* Tap *💳 Supermoney Card* or send _/card_\n` +
      `• *EMIs & Debts:* Tap *📑 EMIs & Dues* or *🤝 Debts*\n` +
...
bot.command("link", async (ctx) => {
  ...
  try {
    ...
    await ctx.reply(`🎉 *Success! Linked to OrbitLife.*\n\nUser ID: \`${userId}\`\n\nYou can now log expenses, mark EMIs paid, update debts, and check balances! 🚀`, { parse_mode: "Markdown", ...mainMenu });
  } catch (err) {
...
async function handleBalanceQuery(ctx) {
  const chatId = ctx.chat.id;
  const userId = await getUserIdByChatId(chatId);
  if (!userId) return ctx.reply("⚠️ Link account first with `/link <USER_ID>`.");
...
bot.hears("🥗 Nutrition & Food", (ctx) => ctx.reply("📸 *Snap a food picture* or type what you ate (e.g., _'2 chapatis with dal and paneer'_).", { parse_mode: "Markdown" }));
bot.hears("⚙️ Account Link", (ctx) => ctx.reply("💡 Use `/link YOUR_FIREBASE_USER_ID` to connect your account."));
...
// ─── COMMAND: /addaccount (e.g. Cash in Hand) ───────────────────────────────
...
// ─── COMMAND: /setbalance (Direct Balance Fix / Correction) ────────────────
bot.command("setbalance", async (ctx) => {
  ...
  if (parts.length < 2) {
    return ctx.reply(
      ...
      "• Or just type in natural language: _'Set SBI balance to 3312.60'_",
      { parse_mode: "Markdown" }
    );
  }
...
// ─── COMMAND: /undo (Revert Last Action) ────────────────────────────────────
...
bot.command("help", async (ctx) => {
  const msg = `💡 *OrbitLife Telegram Full Automation Cheatsheet (₹ INR)*\n\n` +
    `*1. Log Spends & Incomes:*\n` +
    ...
    `• _/emis - List active loans & tenures_\n\n` +
    `*3. Manage Borrow & Lend (Debts):*\n` +
    `• _"Shamveel paid back ₹5,000"_\n` +
    `• _"Lent ₹2,000 to Rahul"_\n` +
    `• _/debts - Check who owes you money_\n\n` +
    `*4. Credit Card & FD:*\n` +
    `• _"Paid ₹10,000 credit card bill"_\n` +
    `• _/card - Check Supermoney limit & available credit_\n\n` +
    `*5. Live Balances & Analytics:*\n` +
    `• _/balance - Check SBI, HDFC & Cash balances_\n` +
    `• _/today or /stats - View daily spend & monthly analytics_\n\n` +
    `*6. Nutrition & Receipts:*\n` +
    `• Snap any receipt photo 🧾 or food photo 🥗!`;
...
bot.on("text", async (ctx) => {
  ...
  try {
    ...
    if (parsed.intent === "PAY_EMI") {
      ...
      const reply = `🏦 *EMI Paid Successfully!*\n\n` +
        `📌 *Loan:* ${res.emiTitle}\n` +
...
bot.launch().then(() => {
  console.log("🚀 OrbitLife Automation Bot is RUNNING with Full Financial Actions!");
}).catch((err) => {
